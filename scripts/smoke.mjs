...
/* --- data ---------------------------------------------------------------- */
eq(apps.length, 18, 'all 18 entries preserved');
eq(new Set(apps.map(a => a.slug)).size, 18, 'slugs are unique');
for (const a of apps) {
...
eq((list.match(/<article class="row"/g) || []).length, 18, 'one row per app');
eq((list.match(/class="open"/g) || []).length, 18, 'one Open control per row');
...
eq(D.footText(apps, '2026-09-11'), '18 live · 0 building · 0 planned · updated 2026-09-11', 'footer counts');
