...
/* --- data ---------------------------------------------------------------- */
eq(apps.length, 17, 'all 17 entries preserved');
eq(new Set(apps.map(a => a.slug)).size, 17, 'slugs are unique');
for (const a of apps) {
...
eq((list.match(/<article class="row"/g) || []).length, 17, 'one row per app');
eq((list.match(/class="open"/g) || []).length, 17, 'one Open control per row');
...
eq(D.footText(apps, '2026-09-11'), '17 live · 0 building · 0 planned · updated 2026-09-11', 'footer counts');
