...
/* --- data ---------------------------------------------------------------- */
eq(apps.length, 15, 'all 14 entries preserved');
eq(new Set(apps.map(a => a.slug)).size, 15, 'slugs are unique');
for (const a of apps) {
...
eq((list.match(/<article class="row"/g) || []).length, 15, 'one row per app');
eq((list.match(/class="open"/g) || []).length, 15, 'one Open control per row');
...
eq(D.footText(apps, '2026-09-11'), '15 live · 0 building · 0 planned · updated 2026-09-11', 'footer counts');
